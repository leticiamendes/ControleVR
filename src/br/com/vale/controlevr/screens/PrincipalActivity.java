...
public class PrincipalActivity extends FragmentActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		TextView textSaldoInfo = (TextView) findViewById(R.id.info);
		typeface = Typeface.createFromAsset(getAssets(), "QuattrocentoSans-Regular.ttf");
		textSaldoInfo.setTypeface(typeface);
		
		String saldo = Util.readStringFile(getApplicationContext(), Util.FILESALDO);
		String valorGasto = Util.readStringFile(getApplicationContext(), Util.FILEVALORGASTO);
		
		double saldoDouble;
		double valorGastoDouble;
		
		try{
			saldoDouble = Double.parseDouble(saldo.substring(2));
		}catch(Exception ex){
			saldoDouble = 0;
		}
		
		try{
			valorGastoDouble = Double.parseDouble(valorGasto.substring(2));
		}catch(Exception ex){
			valorGastoDouble = 0;
		}
		
		Double saldoAtual = saldoDouble - valorGastoDouble;
		textSaldoInfo.append(String.valueOf(saldoAtual));
			...
			public void onItemClick(AdapterView<?> parent, View v, int position, long id) {
				...
				if (desc.size() > 0) {
					for (int i = 0; i < desc.size(); i++) {
						TextView rowTextView = new TextView(PrincipalActivity.this);
